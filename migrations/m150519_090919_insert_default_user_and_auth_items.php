...
class m150519_090919_insert_default_user_and_auth_items extends Migration
{
    ...
    public function safeUp()
    {
        ...
        $this->insert('{{%users}}', [
            'username' => 'admin',
            'password' => Yii::$app->security->generatePasswordHash('Wei6moo2'),
        ...
        $loggedRole = $authManager->createRole('Authenticated');
        $loggedRole->description = 'Zalogowany użytkownik';
        $authManager->add($loggedRole);
        $guestRole = $authManager->createRole('Guest');
        $guestRole->description = 'Nie zalogowany - gość';
        $authManager->add($guestRole);

        $authManager->addChild($loggedRole, $guestRole);
